...
from pacc.errors.reporting import ErrorReporter
from pacc.ui.components import SearchFilter, SelectableItem
from pacc.validation.formats import FormatDetector, JSONValidator
...
def demo_file_scanning():
    ...
    filtered_files = (
        file_filter.add_pattern_filter(["*.py"]).add_exclude_hidden().filter_files(python_files)
    )
...
def demo_error_handling():
    ...
    reporter.report_validation_error(
        "Invalid JSON syntax", file_path=Path("test.json"), line_number=5, validation_type="JSON"
    )
