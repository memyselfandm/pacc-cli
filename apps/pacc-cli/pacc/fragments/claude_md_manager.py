...
from ..errors.exceptions import FileSystemError, ValidationError, SecurityError
...
class CLAUDEmdManager:
    ...
    def _resolve_reference_path(self, ref_path: str, base_file: Path) -> Path:
        ...
        try:
            ...
            if '..' in ref_path:
                ...
                if not str(resolved).startswith(str(Path.home())) and \
                   not str(resolved).startswith(str(self.project_root)):
                    raise SecurityError(
    ...
    def update_section(self, 
        ...
        with self._atomic_file_operation(file_path) as (temp_file, backup_path):
            ...
            if start_pos == -1:
                ...
                new_content = f"{before_section}{start_marker}\n{content}\n{end_marker}{after_section}"
    ...
    def update_section_with_references(self, 
                                     file_path: Path, 
                                     section_name: str, 
                                     content: str,
                                     create_if_missing: bool = True) -> bool:
