...
def check_pypi_name(package_name: str, test_pypi: bool = False) -> Dict[str, any]:
    ...
    try:
        ...
        if e.code == 404:
            ...
            return {"available": None, "error": f"HTTP Error {e.code}: {e.reason}"}
    except Exception as e:
        return {"available": None, "error": f"Error checking package: {e!s}"}
...
def main():
    ...
    if test_result.get("available") is True:
        ...
        print("  ❌ Package already exists on Test PyPI!")
    ...
    if pypi_result.get("available") and test_result.get("available"):
        ...
        print("  ⚠️  Unable to fully verify availability.")
