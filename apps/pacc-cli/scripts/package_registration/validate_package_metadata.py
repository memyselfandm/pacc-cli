...
class PackageMetadataValidator:
    ...
    def check_description_quality(self, description: str) -> Dict[str, Any]:
        ...
        if all(word not in description.lower() for word in ["manager", "tool", "framework", "utility"]):
            if sum(1 for word in generic_words if word in description.lower()) >= 2:
                suggestions.append("Description seems generic - be more specific about functionality")
                score -= 10
    ...
    def check_urls(self, urls: Dict[str, str]) -> Dict[str, Any]:
        ...
        for url_type, url in urls.items():
            ...
            if "github.com" in url and url.endswith(".git"):
                self.suggestions.append(f"Remove .git from {url_type} URL for better web compatibility")
    ...
    def _calculate_coverage(self, metadata: Dict[str, Any]) -> float:
        ...
        all_fields = [
            ...
            "license", "authors", "maintainers", "keywords", "classifiers",
            "urls", "dependencies", "optional-dependencies", "scripts"
        ]
        
        present_fields = sum(1 for field in all_fields if field in metadata and metadata[field])
...
def validate_pyproject_file(pyproject_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "valid": False,
            "score": 0,
            "errors": [f"Error reading pyproject.toml: {str(e)}"],
            "warnings": [],
            "suggestions": []
        }
...
Examples:
  ...
  %(prog)s --suggestions pyproject.toml
    ...
    parser.add_argument(
        '--strict',
        action='store_true',
        help='Use strict validation (warnings become errors)'
    )
