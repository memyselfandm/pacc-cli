...
class TestDeepMergeStrategy:
    ...
    def test_nested_merge(self):
        ...
        new = {
            "mcps": [{"name": "new"}],
            "settings": {"language": "en", "debug": True}
        }
...
class TestClaudeConfigManager:
    ...
    def test_merge_config_with_array_deduplication(self):
        ...
        initial_config = {
            "hooks": [
                {"name": "hook1", "event": "before"},
                {"name": "hook2", "event": "after"}
            ]
        }
        with open(config_path, 'w') as f:
            json.dump(initial_config, f)
    ...
    def test_add_extension_config_invalid_type(self):
        ...
        with pytest.raises(ConfigurationError) as exc_info:
            self.config_manager.add_extension_config(
                "invalid_type", {}, user_level=False
            )
    ...
    def test_validate_config_structure_invalid(self):
        ...
        for invalid_config in invalid_configs:
            with pytest.raises(ValidationError):
                self.config_manager._validate_config_structure(
                    invalid_config, Path("test.json")
                )
