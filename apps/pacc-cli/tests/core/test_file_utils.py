...
class TestDirectoryScanner(unittest.TestCase):
    ...
    def test_find_files_by_extension(self):
        ...
        json_files = self.scanner.find_files_by_extension(self.temp_dir, {".json"}, recursive=True)
...
class TestFileFilter(unittest.TestCase):
    ...
    def test_pattern_filter(self):
        ...
        filtered = (
            self.filter.clear_filters().add_pattern_filter(["test*"]).filter_files(self.files)
        )
    ...
    def test_size_filter(self):
        """Test size filtering."""
        filtered = (
            self.filter.clear_filters().add_size_filter(min_size=500).filter_files(self.files)
        )
