...
from pacc.core.file_utils import FilePathValidator, DirectoryScanner, PathNormalizer
        ...
        with patch('os.getcwd', return_value=str(tmp_path)):
            with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=claude_dir):
                ...
                if platform.system() == "Windows":
                    ...
                    assert any(ext in os.environ.get("PATHEXT", "") for ext in [".EXE", ".BAT", ".CMD"])
...
class TestPlatformCompatibilityMatrix:
    ...
    def test_platform_detection_accuracy(self):
        ...
        assert detected_platform == expected, f"Platform detection mismatch: {detected_platform} != {expected}"
        ...
        if current_platform == "Windows":
            assert detected_shell in ["cmd", "powershell"], f"Unexpected Windows shell: {detected_shell}"
        else:
            assert detected_shell in ["bash", "zsh", "sh", "fish"], f"Unexpected Unix shell: {detected_shell}"
    ...
    def test_path_separator_handling(self, tmp_path):
        ...
        for path_str in mixed_paths:
            ...
            if platform.system() == "Windows":
                ...
                assert "\\" in normalized_str or "/" in normalized_str or len(normalized_str.split(os.sep)) > 1
            else:
                # Unix systems should use forward slashes
                assert "/" in normalized_str or normalized_str in [".", ".."] or not os.sep in normalized_str
    ...
    def test_special_characters_in_paths(self, tmp_path):
        ...
        if created_files:
            ...
            for created_file in created_files:
                assert created_file in discovered, f"Special character file not discovered: {created_file.name}"
    
    def test_long_path_handling(self, tmp_path):
        """Test handling of long file paths."""
        # Create nested directory structure
        long_path_components = ["very", "long", "path", "structure", "with", "many", "nested", "directories", "for", "testing", "path", "length", "limits"]
