...
def performance_plugin_repo(tmp_path):
    ...
    plugins_list = []
    categories = {"agents": 20, "commands": 15, "mcp": 10, "hooks": 8}
...
"""
                ...
                content = {
                    ...
                    "env": {"PERF_TEST": "true", "PLUGIN_ID": str(i)},
                    "capabilities": ["performance_testing"],
                    "performance": {"memory_limit": "64MB", "timeout": 30},
                }
                ...
                (repo_dir / category / f"{plugin_name}.{extension}").write_text(
...
class TestPluginCLIPerformance:
    ...
    def test_plugin_install_command_performance(
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                ), f"Single install took {profiler.duration:.3f}s (should be < 2s)"
                assert (
                    profiler.peak_memory_mb < 50
                ), f"Peak memory: {profiler.peak_memory_mb:.1f}MB (should be < 50MB)"
                ...
                with CLIPerformanceProfiler("Plugin Install Batch") as profiler:
                    args = ["plugin", "install", str(repo_dir), "--all", "--no-interactive"]
                    result = cli.main(args)

                # Performance assertions for batch install
                assert (
                    profiler.duration < 15.0
                ), f"Batch install took {profiler.duration:.3f}s (should be < 15s)"
                assert (
                    profiler.peak_memory_mb < 100
                ), f"Peak memory: {profiler.peak_memory_mb:.1f}MB (should be < 100MB)"
                ...
                assert (
                    throughput > 3
                ), f"Install throughput: {throughput:.1f} plugins/s (should be > 3/s)"

                print("Plugin install performance:")
    ...
    def test_plugin_list_command_performance(
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                ), f"List installed took {profiler.duration:.3f}s (should be < 1s)"
                ...
                assert (
                    profiler.duration < 2.0
                ), f"List available took {profiler.duration:.3f}s (should be < 2s)"
                ...
                ), f"List filtered took {profiler.duration:.3f}s (should be < 0.5s)"
                assert result == 0, "List filtered command should succeed"

                print("Plugin list performance:")
    ...
    def test_plugin_update_command_performance(
        self, performance_plugin_repo, cli_test_environment, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                ), f"Update check took {profiler.duration:.3f}s (should be < 3s)"
                ...
                ), f"Update execute took {profiler.duration:.3f}s (should be < 10s)"
    ...
    def test_plugin_remove_command_performance(
        self, performance_plugin_repo, cli_test_environment, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                assert (
                    profiler.duration < 1.0
                ), f"Single remove took {profiler.duration:.3f}s (should be < 1s)"
                ...
                assert (
                    profiler.duration < 3.0
                ), f"Batch remove took {profiler.duration:.3f}s (should be < 3s)"
                assert result == 0, "Batch remove should succeed"

                print("Plugin remove performance:")
    ...
    def test_plugin_info_command_performance(
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                assert (
                    profiler.duration < 0.5
                ), f"Plugin info took {profiler.duration:.3f}s (should be < 0.5s)"
    ...
    def test_plugin_sync_command_performance(
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                assert (
                    profiler.duration < 2.0
                ), f"Sync check took {profiler.duration:.3f}s (should be < 2s)"
                ...
                ), f"Sync execute took {profiler.duration:.3f}s (should be < 5s)"
                assert result == 0, "Sync execute should succeed"

                print("Plugin sync performance:")
...
"""
                    ...
                    (large_repo / category / f"{plugin_name}.{extension}").write_text(
                        yaml.dump(content)
                    )
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                assert (
                    profiler.duration < 10.0
                ), f"Large repo list took {profiler.duration:.3f}s (should be < 10s)"
                assert (
                    profiler.peak_memory_mb < 150
                ), f"Peak memory: {profiler.peak_memory_mb:.1f}MB (should be < 150MB)"
                assert result == 0
                ...
                assert (
                    profiler.duration < 8.0
                ), f"Large repo subset install took {profiler.duration:.3f}s"
                assert result == 0

                # Calculate performance metrics
                discovery_throughput = len(plugins_list) / profiler.duration
                print("Large repository CLI performance:")
    ...
    def test_rapid_cli_command_sequence(
        self, performance_plugin_repo, cli_test_environment, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                assert (
                    avg_command_time < 2.0
                ), f"Average command time: {avg_command_time:.3f}s (should be < 2s)"

                print("Rapid CLI command sequence performance:")
    ...
    def test_cli_memory_efficiency(self, performance_plugin_repo, cli_test_environment, tmp_path):
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                assert (
                    max_memory_mb < 100
                ), f"Peak memory usage: {max_memory_mb:.1f}MB (should be < 100MB)"
                assert (
                    final_memory_mb < 20
                ), f"Final memory delta: {final_memory_mb:.1f}MB (should be < 20MB)"

                print("CLI memory efficiency:")
                ...
                print("  Memory efficiency: Good" if max_memory_mb < 50 else "Acceptable")
...
class TestPluginCLIIntegration:
        ...
        with patch("os.getcwd", return_value=str(test_env)):
            with patch(
                ...
                for args, description in error_scenarios:
                    ...
                    assert (
                        profiler.duration < 1.0
                    ), f"Error handling took {profiler.duration:.3f}s (should be < 1s)"
                    assert result != 0, f"Command should fail for: {description}"

                avg_error_time = sum(error_handling_times) / len(error_handling_times)
                assert (
                    avg_error_time < 0.5
                ), f"Average error handling: {avg_error_time:.3f}s (should be < 0.5s)"

                print("CLI error handling performance:")
    ...
    def test_cli_concurrent_safety(self, performance_plugin_repo, cli_test_environment, tmp_path):
                    ...
                    try:
                        ...
                        operations = [
                            ...
                            [
                                ...
                                str(repo_dir),
                                "--plugin",
                                f"perf-agents-{operation_id:02d}",
                                "--no-interactive",
                            ],
                            ["plugin", "list", "--installed"],
                        ...
                        results.append({"operation": "error", "success": False, "error": str(e)})

                    return operation_id, results

        # Run concurrent CLI operations
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(run_cli_operation, i) for i in range(4)]
        ...
        ), f"Average concurrent operation time: {avg_operation_time:.3f}s"

        print("CLI concurrent safety test:")
        print("  Concurrent threads: 4")
