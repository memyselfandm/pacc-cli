...
"""
    ...
    dev_server_mcp = {
        ...
        "command": "python",
        "args": ["-m", "dev_server"],
        "env": {"SERVER_PORT": "8080", "DEBUG_MODE": "true", "LOG_LEVEL": "info"},
        "capabilities": ["server_management", "hot_reload", "log_streaming"],
    }
    ...
    file_watcher_hook = {
        ...
        "actions": {
            "lint": {"command": "pylint", "args": ["${file_path}"]},
            "format": {"command": "prettier", "args": ["--write", "${file_path}"]},
            "validate": {"command": "markdownlint", "args": ["${file_path}"]},
        },
    }
...
class TestCompletePluginLifecycle:
    ...
    def test_new_user_plugin_discovery_and_installation(
        self, mock_plugin_repo_structure, mock_claude_environment, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    timer.duration < 2.0
                ), f"Plugin discovery took {timer.duration:.3f}s (should be < 2s)"
                ...
                assert (
                    timer.duration < 1.0
                ), f"Plugin installation took {timer.duration:.3f}s (should be < 1s)"
                ...
                assert settings_data["agents"]["quick-notes"]["path"] == str(
                    repo_dir / "agents" / "quick-notes.md"
                )
                ...
                assert (
                    timer.duration < 3.0
                ), f"Batch installation took {timer.duration:.3f}s (should be < 3s)"
    ...
    def test_plugin_update_workflow(
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    timer.duration < 1.0
                ), f"Update check took {timer.duration:.3f}s (should be < 1s)"
                ...
                assert (
                    timer.duration < 2.0
                ), f"Update application took {timer.duration:.3f}s (should be < 2s)"
...
"""
                (conflicting_repo / "agents" / "quick-notes.md").write_text(
                    conflicting_plugin_content
                )
                ...
                with PerformanceTimer("Conflict Detection") as timer:
                    ...
                    conflict_result = config_manager.install_plugins(
                        conflicting_plugins, conflicting_repo, allow_conflicts=False
                    )

                assert (
                    timer.duration < 1.0
                ), f"Conflict detection took {timer.duration:.3f}s (should be < 1s)"
                ...
                assert (
                    timer.duration < 1.0
                ), f"Conflict resolution took {timer.duration:.3f}s (should be < 1s)"
                ...
                quick_notes_config = settings_data["agents"]["quick-notes"]
                assert quick_notes_config["path"] == str(
                    conflicting_repo / "agents" / "quick-notes.md"
                )
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                with PerformanceTimer("Single Plugin Removal") as timer:
                    removal_result = config_manager.remove_plugin(
                        "quick-notes", plugin_type="agent"
                    )

                assert (
                    timer.duration < 0.5
                ), f"Plugin removal took {timer.duration:.3f}s (should be < 0.5s)"
                ...
                assert (
                    timer.duration < 0.5
                ), f"Batch removal took {timer.duration:.3f}s (should be < 0.5s)"
                ...
                assert (
                    timer.duration < 1.0
                ), f"Complete cleanup took {timer.duration:.3f}s (should be < 1s)"
    ...
    def test_plugin_environment_setup_workflow(self, mock_claude_environment, tmp_path):
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    timer.duration < 0.5
                ), f"Status check took {timer.duration:.3f}s (should be < 0.5s)"
                ...
                assert (
                    timer.duration < 2.0
                ), f"Environment setup took {timer.duration:.3f}s (should be < 2s)"
                ...
                assert (
                    timer.duration < 1.0
                ), f"Environment verification took {timer.duration:.3f}s (should be < 1s)"
                ...
                assert (
                    timer.duration < 1.0
                ), f"Environment reset took {timer.duration:.3f}s (should be < 1s)"
...
"""
                    ...
                    content = {"name": filename, "command": "python", "args": ["-m", filename]}
        ...
        assert (
            timer.duration < 5.0
        ), f"Large repo discovery took {timer.duration:.3f}s (should be < 5s)"
        ...
        assert (
            throughput > 20
        ), f"Discovery throughput: {throughput:.1f} plugins/s (should be > 20/s)"
    ...
    def test_batch_installation_performance(
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    throughput > 2
                ), f"Installation throughput: {throughput:.1f} plugins/s (should be > 2/s)"
    ...
    def test_configuration_update_performance(
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    avg_time < 0.1
                ), f"Average config update took {avg_time:.3f}s (should be < 0.1s)"
                assert total_time < 2.0, f"Total update time {total_time:.3f}s (should be < 2s)"

                throughput = update_count / total_time
                assert (
                    throughput > 10
                ), f"Update throughput: {throughput:.1f} updates/s (should be > 10/s)"
    ...
    def test_concurrent_plugin_operations(
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    timer.duration < 10.0
                ), f"Concurrent operations took {timer.duration:.3f}s (should be < 10s)"
                ...
                successful_ops = [
                    r for r in all_results if r[0] in ["install", "remove"] and r[3] is True
                ]
                total_expected = 4 * 5 * 2  # 4 threads * 5 cycles * 2 operations
                assert len(successful_ops) == total_expected

                print(
                    f"Completed {len(successful_ops)} concurrent operations in {timer.duration:.3f}s"
                )
    ...
    def test_memory_usage_under_load(
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    max_memory_mb < 50
                ), f"Peak memory usage: {max_memory_mb:.1f}MB (should be < 50MB)"
                assert (
                    final_memory_mb < 10
                ), f"Final memory delta: {final_memory_mb:.1f}MB (should be < 10MB)"
    ...
    def test_error_recovery_under_stress(
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                for scenario in error_scenarios:
                    try:
                        ...
                        recovery_results.append(
                            {"scenario": scenario, "duration": timer.duration, "success": True}
                        )
                ...
                assert (
                    len(successful_recoveries) >= len(error_scenarios) - 1
                ), "Most scenarios should recover successfully"

                # Verify recovery is reasonably fast
                avg_recovery_time = sum(r["duration"] for r in successful_recoveries) / len(
                    successful_recoveries
                )
                assert (
                    avg_recovery_time < 2.0
                ), f"Average recovery time: {avg_recovery_time:.3f}s (should be < 2s)"
