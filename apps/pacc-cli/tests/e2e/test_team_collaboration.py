...
class TestTeamPluginSync:
    ...
    def test_initial_team_setup_sync(self, team_plugin_repository, development_team, tmp_path):
        ...
        with patch('os.getcwd', return_value=str(tmp_path)):
            ...
            with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=alice.claude_dir):
            ...
            for member_name in ["bob", "charlie", "diana"]:
                ...
                with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=member.claude_dir):
                    member_repo_manager = member.get_repo_manager()
                    member_config_manager = member.get_config_manager()
                    
                    # Sync from shared repository
                    start_time = time.perf_counter()
                    sync_result = member_config_manager.sync_with_repository(repo_dir, strategy="team_sync")
                    sync_time = time.perf_counter() - start_time
                    
                    sync_times.append(sync_time)
                    
                    assert sync_time < 2.0, f"{member_name} sync took {sync_time:.3f}s (should be < 2s)"
    ...
    def test_plugin_conflict_resolution_across_team(self, team_plugin_repository, development_team, tmp_path):
        ...
        with patch('os.getcwd', return_value=str(tmp_path)):
            ...
            with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=bob.claude_dir):
                ...
                conflict_result = bob_config_manager.sync_with_repository(
                    repo_dir, 
                    strategy="detect_conflicts"
                )
                conflict_time = time.perf_counter() - start_time
                
                assert conflict_time < 1.0, f"Conflict detection took {conflict_time:.3f}s (should be < 1s)"
                ...
                code_reviewer_conflict = next(
                    (c for c in conflicts if c['plugin_name'] == 'code-reviewer'), 
                    None
                )
            ...
            with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=bob.claude_dir):
                ...
                override_result = bob_config_manager.sync_with_repository(
                    repo_dir,
                    strategy="team_override",
                    conflict_resolution="use_team_version"
                )
                override_time = time.perf_counter() - start_time
                
                assert override_time < 1.0, f"Conflict resolution took {override_time:.3f}s (should be < 1s)"
    ...
    def test_collaborative_plugin_development(self, team_plugin_repository, development_team, tmp_path):
...
"""
                (dev_plugin_dir / "agents" / "performance-analyzer.md").write_text(new_plugin_content)
            ...
            with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=alice.claude_dir):
                ...
                assert package_time < 1.0, f"Package creation took {package_time:.3f}s (should be < 1s)"
            ...
            for member_name in ["bob", "charlie"]:
                ...
                with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=member.claude_dir):
                    ...
                    assert review_time < 1.0, f"{member_name} review install took {review_time:.3f}s"
                    assert review_result['success'] is True
            
            # Step 4: Collect feedback and finalize plugin
            with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=alice.claude_dir):
                # Simulate positive team feedback and finalization
                final_plugin_content = new_plugin_content.replace(
                    "version: 0.1.0-dev",
                    "version: 1.0.0"
                ).replace(
                    'status: "development"',
    ...
    def test_team_plugin_rollback_scenario(self, team_plugin_repository, development_team, tmp_path):
        ...
        with patch('os.getcwd', return_value=str(tmp_path)):
            ...
            broken_plugin = original_plugin.replace(
                "version: 1.0.0",
                "version: 1.1.0"
            ).replace(
                "# Code Reviewer Agent",
            ...
            with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=alice.claude_dir):
                ...
                rollback_plan = alice_config_manager.create_team_rollback_plan(
                    ...
                    affected_plugins=["code-reviewer"]
                )
                rollback_plan_time = time.perf_counter() - start_time
                
                assert rollback_plan_time < 1.0, f"Rollback plan creation took {rollback_plan_time:.3f}s"
            ...
            for member_name, member in team.items():
                with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=member.claude_dir):
                    ...
                    start_time = time.perf_counter()
                    rollback_result = member_config_manager.execute_rollback(rollback_plan['plan_id'])
...
class TestTeamCollaborationPerformance:
        ...
        with patch('os.getcwd', return_value=str(tmp_path)):
            ...
            for member_name, member in large_team.items():
                with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=member.claude_dir):
                    ...
                    assert sync_time < 5.0, f"{member_name} sync took {sync_time:.3f}s (should be < 5s)"
                    assert sync_result['success'] is True
            ...
            print(f"Large team sync performance:")
    ...
    def test_concurrent_team_operations(self, team_plugin_repository, development_team, tmp_path):
        ...
        with patch('os.getcwd', return_value=str(tmp_path)):
            ...
            assert total_time < 10.0, f"Concurrent operations took {total_time:.3f}s (should be < 10s)"
        ...
        with patch('os.getcwd', return_value=str(tmp_path)):
            ...
            for member_name, member in stress_team.items():
                with patch('pacc.core.project_config.ProjectConfigValidator._find_claude_dir', return_value=member.claude_dir):
                    ...
                    assert discovery_time < 3.0, f"{member_name} discovery took {discovery_time:.3f}s"
            ...
            assert all(count == 50 for count in plugin_counts), "All members should process 50 plugins"
            
            print(f"Stress test completed:")
            print(f"  Team size: {len(stress_team)} members")
            print(f"  Plugin count: 50 plugins")
