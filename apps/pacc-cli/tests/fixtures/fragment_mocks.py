...
from pacc.fragments.installation_manager import InstallationResult, FragmentSource
from pacc.fragments.storage_manager import FragmentLocation
from pacc.fragments.update_manager import UpdateResult
...
"""
    ...
    def _generate_mock_fragment_content(self, name: str, fragment_type: str, version: str, **kwargs):
...
class MockFragmentStorageManager:
    ...
    def store_fragment(self, name: str, content: str, fragment_type: str, 
                      target_type: str = "project", metadata: Optional[Dict] = None) -> FragmentLocation:
        ...
        location = FragmentLocation(
            ...
            fragment_path=self.project_root / ".claude" / "pacc" / "fragments" / fragment_type / f"{name}.{'md' if fragment_type in ['agent', 'command'] else 'json'}"
        )
    ...
    def get_fragment_location(self, name: str, location_type: str = "project") -> FragmentLocation:
        ...
        return FragmentLocation(
            ...
            base_path=self.project_root / ".claude" / "pacc" / "fragments",
            fragment_path=self.project_root / ".claude" / "pacc" / "fragments" / "agent" / f"{name}.md"
        )
...
class MockFragmentInstallationManager:
    ...
    def install_from_source(self, source_input: str, target_type: str = "project",
                           interactive: bool = False, install_all: bool = False,
                           force: bool = False, dry_run: bool = False) -> InstallationResult:
        ...
        result = InstallationResult(
            ...
            error_message="",
            dry_run=dry_run,
            changes_made=[f"Installed {name}" for name in mock_fragments.keys()] if not dry_run else []
        )
    ...
    def _generate_mock_installation_result(self, source_input: str, target_type: str) -> Dict[str, Dict[str, Any]]:
        ...
        if "deterministic" in source_input:
            return {
                ...
                "test-simple-command": {"type": "command", "version": "1.0.0", "complexity": "simple"},
                "test-complex-command": {"type": "command", "version": "1.0.0", "complexity": "complex"},
                "test-deterministic-hook": {"type": "hook", "version": "1.0.0", "complexity": "simple"},
                "test-complex-hook": {"type": "hook", "version": "1.0.0", "complexity": "complex"}
            }
        elif "edge_case" in source_input:
            return {
                "minimal-test-agent": {"type": "agent", "version": "1.0.0", "edge_case": "minimal_content"},
                "special-chars-agent": {"type": "agent", "version": "1.0.0", "edge_case": "special_characters"},
                "no-params-command": {"type": "command", "version": "1.0.0", "edge_case": "no_parameters"},
                "minimal-hook": {"type": "hook", "version": "1.0.0", "edge_case": "minimal_configuration"}
            }
            ...
            return {
                ...
                "integrated-command": {"type": "command", "version": "1.0.0", "dependency_level": 2}
            }
        else:
            # Single fragment
            ...
            return {
                fragment_name: {"type": fragment_type, "version": "1.0.0"}
            }
...
""",
            ...
            "simple_hook": {
                ...
                "content": json.dumps({
                    ...
                    "action": {
                        "type": "log",
                        "message": "Test hook executed - deterministic"
                    }
