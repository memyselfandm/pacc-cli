...
"""
    ...
    def _create_test_mcp(self, name: str, command: str = None) -> Path:
        ...
        mcp_data = {"mcpServers": {name: {"command": command, "args": [], "env": {}}}}
...
class TestEndToEndConversionWorkflow(ConversionIntegrationTestCase):
    ...
    def test_git_repository_integration(self):
        ...
        extensions = self.converter.scan_extensions(self.test_project_dir)
        result = self.converter.convert_to_plugin(
            extensions=extensions, plugin_name="git_test_plugin", destination=self.output_dir
        )
        ...
        repo_created = self.repo_manager.create_plugin_repository(
            plugin_path, plugin_metadata, init_git=True
        )
...
class TestBatchConversion(ConversionIntegrationTestCase):
    ...
    def test_batch_conversion_multiple_extensions(self):
        ...
        self.assertGreaterEqual(
            len(result.converted_extensions), 6, "Should convert multiple extensions"
        )
...
class TestGitPushAuthentication(ConversionIntegrationTestCase):
    ...
    def test_https_token_authentication(self, mock_run):
        ...
        (plugin_path / "plugin.json").write_text(
            json.dumps(
                {"name": "https_test_plugin", "version": "1.0.0", "description": "Test plugin"}
            )
        )
        ...
        success = self.repo_manager.create_plugin_repository(
            plugin_path, {"name": "https_test_plugin"}, init_git=True
        )
        ...
        push_success = self.repo_manager.push_plugin(
            plugin_path, "https://github.com/test/repo.git", auth=auth
        )
    ...
    def test_ssh_authentication(self, mock_run):
        ...
        (plugin_path / "plugin.json").write_text(
            json.dumps(
                {"name": "ssh_test_plugin", "version": "1.0.0", "description": "SSH test plugin"}
            )
        )
        ...
        self.repo_manager.create_plugin_repository(
            plugin_path, {"name": "ssh_test_plugin"}, init_git=True
        )
        ...
        push_success = self.repo_manager.push_plugin(plugin_path, "git@github.com:test/repo.git")
    ...
    def test_authentication_error_handling(self, mock_run):
        ...
        (plugin_path / "plugin.json").write_text(
            json.dumps(
                {"name": "auth_fail_plugin", "version": "1.0.0", "description": "Auth failure test"}
            )
        )
        ...
        self.repo_manager.create_plugin_repository(
            plugin_path, {"name": "auth_fail_plugin"}, init_git=True
        )
        ...
        push_success = self.repo_manager.push_plugin(
            plugin_path, "https://github.com/test/private-repo.git"
        )
...
class TestPerformanceTesting(ConversionIntegrationTestCase):
    ...
    def test_large_extension_conversion_performance(self):
        ...
        self.assertGreaterEqual(
            len(extensions), num_extensions * 0.9, "Should find most created extensions"
        )
...
class TestCLIIntegration(ConversionIntegrationTestCase):
    ...
    def test_plugin_push_command_integration(self):
        ...
        (plugin_path / "plugin.json").write_text(
            json.dumps(
                {"name": "push_test_plugin", "version": "1.0.0", "description": "Push test plugin"}
            )
        )
        ...
        with patch.object(cli, "_confirm_action", return_value=True), patch(
            "pacc.plugins.converter.PluginPusher.push_plugin", return_value=True
        ):
