...
""")
    ...
    def test_validation_workflow_with_folder_structure(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            extensions_config = {"hooks": [], "commands": [], "agents": [], "mcp": []}
        ...
        pacc_config = {
            ...
            "extensions": {
                "hooks": [
                    {
                        "name": "test-hook",
                        "source": "./extensions/hooks/test-hook.json",
                        "version": "1.0.0",
                    }
                ],
                "commands": [
                    {
                        "name": "test-command",
                        "source": "./extensions/commands/test-command.md",
                        "version": "1.0.0",
                    }
                ],
            },
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            for ext_type, detected_files in detected_extensions.items():
                if ext_type in validation_results:
                    ...
                    assert (
                        len(overlap) > 0
                    ), f"No overlap between detection and validation for {ext_type}"
...
class TestEdgeCasesIntegration:
    ...
    def test_mixed_file_permissions(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            accessible_hook.write_text(
                json.dumps(
                    {"name": "accessible-hook", "version": "1.0.0", "events": ["PreToolUse"]}
                )
            )
            ...
            restricted_hook.write_text(
                json.dumps(
                    {"name": "restricted-hook", "version": "1.0.0", "events": ["PreToolUse"]}
                )
            )
    ...
    def test_circular_symlink_handling(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            normal_file.write_text(
                json.dumps({"name": "normal-hook", "version": "1.0.0", "events": ["PreToolUse"]})
            )
            ...
            try:
                if hasattr(os, "symlink"):
                    ...
                    assert (
                        duration < 10.0
                    ), f"Circular symlink caused infinite loop: {duration:.2f}s"
...
"""
            ...
            if any(extensions_config.values()):
                ...
                try:
                    pacc_json = project_dir / "pacc.json"
                    pacc_json.write_text(
                        json.dumps(pacc_config, ensure_ascii=False, indent=2), encoding="utf-8"
                    )
