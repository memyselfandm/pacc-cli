...
from ..fixtures.sample_fragments import (
    SampleFragmentFactory,
...
class TestFragmentSampleIntegration:
    ...
    def setup_method(self):
        ...
        # Initialize managers
        self.installation_manager = FragmentInstallationManager(
            project_root=self.project_root
        )
        self.storage_manager = FragmentStorageManager(
            project_root=self.project_root
        )
        self.update_manager = FragmentUpdateManager(
            project_root=self.project_root
        )
        self.claude_md_manager = CLAUDEmdManager(
            project_root=self.project_root
        )
...
"""
    ...
    def test_deterministic_collection_install_consistency(self):
        ...
        for i in range(3):
            ...
            assert result.installed_count == 6, f"Expected 6 fragments, got {result.installed_count}"
        ...
        for i, result in enumerate(results[1:], 1):
            ...
            assert result.installed_count == first_result.installed_count, f"Run {i+1} count differs"
            ...
            assert len(result.installed_fragments) == len(first_result.installed_fragments), \
                f"Run {i+1} has different fragment count"
            
            for fragment_name in first_result.installed_fragments:
                assert fragment_name in result.installed_fragments, \
...
"""
    ...
    def test_install_update_remove_workflow(self):
        ...
        assert install_result.installed_count == 6, f"Expected 6 fragments, got {install_result.installed_count}"
        
        # Verify fragments are actually installed
        stored_fragments = self.storage_manager.list_installed_fragments()
        assert len(stored_fragments) == 6, f"Storage shows {len(stored_fragments)} fragments, expected 6"
        ...
        assert len(final_fragments) == expected_remaining, \
        ...
        for removed_fragment in fragments_to_remove:
            assert removed_fragment not in [f["name"] for f in final_fragments], \
    ...
    def test_edge_case_fragments_consistency(self):
        ...
        for run in range(2):
            ...
            assert "special_characters" in special_fragment.get("metadata", {}).get("test_metadata", {})
    ...
    def test_dependency_resolution_workflow(self):
        ...
        assert result.installed_count == 3, f"Expected 3 fragments with dependencies"
    ...
    def test_version_management_with_samples(self):
        ...
        stored_fragments = self.storage_manager.list_installed_fragments()
        v1_fragment = next((f for f in stored_fragments if f["name"] == "versioned-test-agent"), None)
        ...
        v11_update = self.update_manager.update_fragment(
            "versioned-test-agent",
            str(v11_path)
        )
        ...
        v11_fragment = next((f for f in stored_fragments_after if f["name"] == "versioned-test-agent"), None)
        ...
        final_fragments = self.storage_manager.list_installed_fragments()
        v2_fragment = next((f for f in final_fragments if f["name"] == "versioned-test-agent"), None)
            ...
            if collection_path.is_dir():
                for fragment_path in collection_path.rglob("*.md"):
                    ...
                    assert validation_result.is_valid, \
                        f"Validation failed for {fragment_path} in {collection_name}: {validation_result.errors}"
    ...
    def test_dry_run_consistency(self):
        ...
        assert len(initial_fragments) == len(final_fragments), "Fragment count changed during dry run"
    ...
    def test_performance_consistency(self):
        ...
        assert all(t < 10.0 for t in install_times), \
