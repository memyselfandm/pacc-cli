...
from pacc.plugins.config import PluginConfigManager, AtomicFileWriter
from pacc.plugins.repository import PluginRepositoryManager, PluginRepo, UpdateResult
from pacc.plugins.discovery import PluginScanner, RepositoryInfo, PluginInfo, discover_plugins
from pacc.cli import PACCCli
from pacc.errors.exceptions import PACCError, ConfigurationError, ValidationError
...
"""
    ...
    def test_complete_install_discover_enable_list_workflow(self):
        ...
        repo_path = self.create_test_plugin_repo("testowner", "testrepo", [
            {
                "name": "utility-plugin",
        ...
        with patch.object(self.repo_manager, '_get_current_commit_sha', return_value="abc123def"):
            ...
            success = self.config_manager.add_repository("testowner", "testrepo", {
            ...
            assert len(repo_info.plugins) == 2, f"Expected 2 plugins, found {len(repo_info.plugins)}"
            ...
            enable1_success = self.config_manager.enable_plugin("testowner/testrepo", "utility-plugin")
            ...
            assert "utility-plugin" in repo_entry["plugins"], "utility-plugin not in repo plugins list"
            ...
            assert "testowner/testrepo" in settings_data["enabledPlugins"], "Repository not in enabledPlugins"
    ...
    def test_multi_plugin_repository_workflow(self):
        ...
        repo_path = self.create_test_plugin_repo("multiowner", "multirepo", [
            {
                "name": "commands-only",
        ...
        with patch.object(self.repo_manager, '_get_current_commit_sha', return_value="multi123"):
            ...
            repo_info = self.scanner.scan_repository(repo_path)
            assert len(repo_info.valid_plugins) == 3, f"Expected 3 plugins, found {len(repo_info.valid_plugins)}"
            
            # Enable only specific plugins (not all)
            enable_success = self.config_manager.enable_plugin("multiowner/multirepo", "commands-only")
            ...
            disable_success = self.config_manager.disable_plugin("multiowner/multirepo", "commands-only")
    ...
    def test_error_scenarios_and_recovery(self):
        ...
        with patch('pacc.plugins.config.AtomicFileWriter.write_json', side_effect=OSError("Disk full")):
        ...
        assert "No plugins found" in validation_result.error_message, "Should indicate no plugins found"
    ...
    def test_cross_component_interactions(self):
        ...
        repo_path = self.create_test_plugin_repo("complex", "repo", [
            {
                "name": "full-plugin",
        ...
        with patch.object(self.repo_manager, '_get_current_commit_sha', return_value="complex123"):
            ...
            assert len(commands_meta) == 2, f"Expected 2 commands metadata, got {len(commands_meta)}"
            ...
            assert "full-plugin:subdir:nested" in command_names, "Nested command namespace incorrect"
            ...
            assert "full-plugin" in settings_data["enabledPlugins"]["complex/repo"], "Plugin not enabled"
    ...
    def test_configuration_atomicity_and_rollback(self):
        ...
        assert "rollback/test1" not in config_data.get("repositories", {}), "Rolled back repo still present"
        assert "rollback/test1" not in settings_data.get("enabledPlugins", {}), "Rolled back settings still present"
    ...
    def test_performance_validation(self):
        ...
        assert len(repo_info.valid_plugins) == 50, f"Expected 50 plugins, found {len(repo_info.valid_plugins)}"
        
        # Test configuration update performance
        start_time = time.time()
        
        with patch.object(self.repo_manager, '_get_current_commit_sha', return_value="perf123"):
            success = self.config_manager.add_repository("performance", "test", {
                "plugins": [f"perf-plugin-{i:02d}" for i in range(50)]
            })
        ...
        with self.config_manager.transaction():
            for i in range(10):  # Enable first 10 plugins
                success = self.config_manager.enable_plugin("performance/test", f"perf-plugin-{i:02d}")
        ...
        assert len(enabled_plugins) == 10, f"Expected 10 enabled plugins, found {len(enabled_plugins)}"
        
        logger.info(f"Performance validation passed - Discovery: {discovery_time:.3f}s, Config: {config_time:.3f}s, Bulk Enable: {bulk_enable_time:.3f}s")
...
class TestPluginIntegrationWithCLI:
    ...
    def test_cli_plugin_workflow_integration(self, mock_home):
        ...
        with patch('pacc.plugins.config.PluginConfigManager.enable_plugin', return_value=True) as mock_enable, \
             patch('pacc.plugins.config.PluginConfigManager.disable_plugin', return_value=True) as mock_disable:
...
class TestPluginDependencyResolution:
    ...
    def test_team_config_synchronization(self):
        ...
        assert sync_result["installed_count"] == 3, f"Expected 3 repos installed, got {sync_result['installed_count']}"
        assert sync_result["failed_count"] == 0, f"Expected no failures, got {sync_result['failed_count']}"
        ...
        for repo in expected_repos:
            ...
            assert repo in settings_data["enabledPlugins"], f"Repository {repo} not in enabledPlugins"
        
        # Verify specific plugins are enabled
        assert "formatter" in settings_data["enabledPlugins"]["team/utilities"], "formatter not enabled"
        assert "linter" in settings_data["enabledPlugins"]["team/utilities"], "linter not enabled"
        assert "code-reviewer" in settings_data["enabledPlugins"]["team/agents"], "code-reviewer not enabled"
        assert "git-hooks" in settings_data["enabledPlugins"]["external/tools"], "git-hooks not enabled"
    ...
    def test_plugin_conflict_detection(self):
        ...
        assert "shared-name" in settings_data["enabledPlugins"]["repo1/test"], "Plugin from repo1 not enabled"
        assert "shared-name" in settings_data["enabledPlugins"]["repo2/test"], "Plugin from repo2 not enabled"
