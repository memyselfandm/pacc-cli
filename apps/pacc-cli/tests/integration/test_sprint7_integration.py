...
class TestSecurityIntegration:
    ...
    def test_command_scanner_basic_functionality(self):
        ...
        assert any(
            issue.threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]
    ...
    def test_enhance_validation_with_security(self, tmp_path):
        ...
        plugin_content = {"name": "test-plugin", "commands": ["echo 'safe command'"]}
        plugin_file.write_text(json.dumps(plugin_content))

        # Create base validation result
        base_result = ValidationResult(
            is_valid=True, file_path=str(plugin_file), extension_type="hooks"
        )
...
class TestMarketplaceIntegration:
    ...
    def test_registry_config_validation(self):
        ...
        with pytest.raises(ValueError):
            RegistryConfig(name="invalid", url="not-a-url", registry_type=RegistryType.PUBLIC)
...
""")
        ...
        is_safe, issues = sandbox_manager.validate_plugin_in_sandbox(plugin_dir, "commands")
