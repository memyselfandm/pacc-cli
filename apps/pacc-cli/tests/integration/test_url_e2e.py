...
class TestURLInstallationE2E:
    ...
    def test_url_install_hooks_extension(self):
        ...
        hook_content = {
            ...
            "commands": [{"matchers": ["*"], "command": "echo 'test hook'"}],
        }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("pacc.sources.url.asyncio.run") as mock_asyncio_run:
                ...
                extensions = handler.process_source(
                    "https://example.com/test_mcp.zip", extension_type="mcp"
                )
...
@pytest.mark.skipif(
    not URLSourceHandler().available, reason="aiohttp not available for URL downloads"
)
class TestURLInstallationWithDependencies:
    ...
    def test_url_downloader_configuration(self):
        ...
        handler = URLSourceHandler(max_file_size_mb=50, timeout_seconds=120, show_progress=False)
