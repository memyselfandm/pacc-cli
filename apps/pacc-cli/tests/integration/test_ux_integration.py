...
class TestEnvironmentToPluginWorkflow:
    ...
    def setup_method(self):
        ...
        self.config_manager = PluginConfigManager(
            plugins_dir=self.plugins_dir, settings_path=settings_path
        )
    ...
    def test_complete_onboarding_workflow(self, mock_home):
        ...
        with patch.object(
            self.env_manager, "detect_platform", return_value=Platform.MACOS
        ), patch.object(self.env_manager, "detect_shell", return_value=Shell.ZSH), patch.object(
            self.env_manager, "_is_containerized", return_value=False
        ):
            ...
            if success:  # Only check content if setup succeeded
                ...
                assert (
                    "export ENABLE_PLUGINS=true" in zshrc_content
                ), "ENABLE_PLUGINS export not found"
            else:
            ...
            with patch.object(
                self.config_manager, "add_repository", return_value=True
                ...
                enable_success = self.config_manager.enable_plugin(
                    "user/test-plugins", "utility-plugin"
                )
    ...
    def test_cross_platform_environment_setup(self, mock_home):
        ...
        for platform_type, shell_type, config_file, expected_export in test_scenarios:
            ...
            with patch.object(
                self.env_manager, "detect_platform", return_value=platform_type
            ), patch.object(
                self.env_manager, "detect_shell", return_value=shell_type
                ...
                assert (
                    expected_export in content
                ), f"Expected export not found for {shell_type.value}, got: {content}"
    ...
    def test_environment_error_recovery(self, mock_home):
        ...
        with patch.object(
            self.env_manager, "detect_platform", return_value=Platform.LINUX
    ...
    def test_environment_verification_comprehensive(self):
        ...
        for env_set, env_value, expected_success, expected_message in test_cases:
            ...
            with patch.dict(
                os.environ, {} if not env_set else {"ENABLE_PLUGINS": env_value}, clear=False
            ):
    ...
    def test_slash_command_parameter_passing(self):
        ...
        with patch("pathlib.Path.home", return_value=self.temp_dir):
            ...
            test_cases = [
                ...
                (
                    ["plugin", "install", "owner/repo", "--enable"],
                    {"repo_url": "owner/repo", "enable": True},
                ),
                (
                    ["plugin", "install", "owner/repo", "--all"],
                    {"repo_url": "owner/repo", "all": True},
                ),
                (["plugin", "list"], {}),
...
class TestPerformanceValidation:
    ...
    def test_environment_setup_performance(self, mock_home):
        ...
        with patch.object(
            ...
            assert success2, f"Second setup failed: {message2}"
            assert (
                second_setup_time < 0.5
            ), f"Second setup took {second_setup_time:.2f}s, expected < 0.5s"

        logger.info(
            f"Environment setup performance: {setup_time:.3f}s, second run: {second_setup_time:.3f}s"
        )
    ...
    def test_new_user_onboarding_flow(self, mock_home):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                enable_success = self.config_manager.enable_plugin(
                    "awesome/claude-plugins", "productivity-tools"
                )
            ...
            with patch.object(
                ...
                for owner, repo, plugins in team_plugins:
                    ...
                    for plugin in plugins:
                        plugin_success = self.config_manager.enable_plugin(
                            f"{owner}/{repo}", plugin
                        )
...
class TestMemoryAndResourceUsage:
    ...
    def test_memory_usage_reasonable(self):
        ...
        assert (
            memory_increase < 10
        ), f"Memory usage increased by {memory_increase:.1f}MB, expected < 10MB"

        logger.info(
            f"Memory usage test - Baseline: {baseline_memory:.1f}MB, Final: {final_memory:.1f}MB, Increase: {memory_increase:.1f}MB"
        )
    ...
    def test_file_handle_cleanup(self):
        ...
        logger.info(
            f"File handle test - Baseline: {baseline_fds}, Final: {final_fds}, Increase: {fd_increase}"
        )
