...
from ..fixtures.sample_fragments import SampleFragmentFactory, create_comprehensive_test_suite
...
"""
        ...
        pacc_config = {
            "name": "fragment-benchmark-project",
            "version": "1.0.0",
            "fragments": {}
        }
    ...
    def benchmark_fragment_validation(self) -> List[BenchmarkResult]:
        ...
        for collection_name, description in collections_to_test:
            ...
            fragment_files.extend([f for f in collection_path.rglob("*.json") if "collection" not in f.name])
            ...
            result = self.benchmark_operation(
                f"validate_{collection_name}_collection",
                validate_collection
            )
    ...
    def benchmark_fragment_installation(self) -> List[BenchmarkResult]:
        ...
        for collection_name, description, install_all in collections_to_test:
            ...
            result = self.benchmark_operation(
                f"install_{collection_name}_collection",
                install_collection
            )
            result.metadata.update({
                "description": description,
        ...
        def install_single_fragment():
            ...
            fragment_path = self.sample_collections["deterministic"] / "agents" / "test-simple-agent.md"
            return self.installation_manager.install_from_source(
                str(fragment_path),
                target_type="project"
            )
        
        single_result = self.benchmark_operation(
            "install_single_fragment",
            install_single_fragment
        )
        single_result.metadata.update({
            "description": "Single fragment installation performance",
            "fragment_type": "agent"
        })
    ...
    def benchmark_fragment_storage(self) -> List[BenchmarkResult]:
        ...
        list_result = self.benchmark_operation(
            "list_installed_fragments",
            list_fragments
        )
    ...
    def benchmark_fragment_updates(self) -> List[BenchmarkResult]:
        ...
        def update_fragment():
            ...
            return self.update_manager.update_fragment(
                "versioned-test-agent",
                str(v11_path)
            )
        
        update_result = self.benchmark_operation(
        ...
        try:
            ...
            for suite_name, benchmark_func in benchmark_suites.items():
                ...
                for result in suite_results:
                    print(f"    {result.operation}: {result.mean_time:.4f}s avg, {result.throughput:.2f} ops/sec")
    ...
    def generate_performance_report(self, results: Dict[str, List[BenchmarkResult]]) -> Dict[str, Any]:
    ...
    def test_validation_performance_benchmarks(self):
        ...
        try:
            ...
            for result in validation_results:
                ...
                assert result.mean_time < 10.0, f"Validation too slow: {result.operation} took {result.mean_time}s"
                
                # Consistency check
                cv = result.metadata.get("coefficient_of_variation", 0)
                assert cv < 50, f"High variance in {result.operation}: {cv}% coefficient of variation"
    ...
    def test_installation_performance_benchmarks(self):
        ...
        try:
            ...
            for result in installation_results:
                assert result.iterations > 0, f"No successful iterations for {result.operation}"
                assert result.mean_time > 0, f"Invalid timing for {result.operation}"
                assert result.mean_time < 30.0, f"Installation too slow: {result.operation} took {result.mean_time}s"
                
                # Throughput check
                assert result.throughput > 0.01, f"Very low throughput for {result.operation}: {result.throughput} ops/sec"
...
if __name__ == "__main__":
    ...
    benchmarks = FragmentPerformanceBenchmarks(
        PerformanceTestSuite(iterations=10, warmup_runs=3)
    )
    ...
    print(f"  Fastest: {report['summary']['fastest_operation']['operation']} - {report['summary']['fastest_operation']['mean_time']:.4f}s")
    print(f"  Slowest: {report['summary']['slowest_operation']['operation']} - {report['summary']['slowest_operation']['mean_time']:.4f}s")