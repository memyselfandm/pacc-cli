...
def plugin_benchmark_repo(tmp_path) -> Path:
    ...
    for config in plugin_configs:
        ...
        for i in range(count):
            ...
            plugins_list.append(
                {
                    ...
                    "version": "1.0.0",
                    "benchmark": {"category": category, "size": size, "index": i},
                }
            )
            ...
            if category in ["agents", "commands"]:
                # Pre-calculate strings to avoid f-string backslash issues
                expected_time = (
                    "< 50ms" if size == "small" else "< 100ms" if size == "medium" else "< 200ms"
                )
                description_text = (
                    "This is a comprehensive plugin with extensive documentation and features. "
                    * content_multiplier
                )
                features_text = "- Feature implementation with detailed description\n" * (
                    5 * content_multiplier
                )
                config_text = (
                    "Configuration section with detailed options and explanations. "
                    * content_multiplier
                )
                ...
                usage_text = "".join(usage_examples)
...
"""
                ...
                (category_dir / f"{plugin_name}.{extension}").write_text(
...
def benchmark_claude_env(tmp_path) -> Path:
    ...
    settings = {
        ...
        "commands": {},
        "mcp": {"servers": {}},
        "benchmark": {"mode": "performance_testing", "start_time": time.time()},
    }
...
class TestPluginDiscoveryPerformance:
    ...
    def test_plugin_discovery_scalability(self, plugin_benchmark_repo):
        ...
        assert (
            throughput > 50
        ), f"Discovery throughput: {throughput:.1f} plugins/s (should be > 50/s)"
        ...
        discovery_checkpoint = next(
            c for c in profiler.checkpoints if c["name"] == "discovery_complete"
        )
        validation_checkpoint = next(
            c for c in profiler.checkpoints if c["name"] == "validation_complete"
        )
        ...
        assert (
            validation_time < 0.5
        ), f"Validation overhead: {validation_time:.3f}s (should be < 0.5s)"

        print("Plugin Discovery Performance:")
    ...
    def test_plugin_discovery_by_type(self, plugin_benchmark_repo):
        ...
        avg_throughput = sum(result["throughput"] for result in type_results.values()) / len(
            type_results
        )
        ...
        assert (
            avg_throughput > 40
        ), f"Average type throughput: {avg_throughput:.1f} (should be > 40)"

        print("Discovery by Type Performance:")
        for plugin_type, result in type_results.items():
            print(
                f"  {plugin_type}: {result['count']} plugins, {result['duration']:.3f}s, {result['throughput']:.1f}/s"
            )
    ...
    def test_manifest_parsing_performance(self, plugin_benchmark_repo):
        ...
        assert (
            avg_throughput > 400
        ), f"Parse throughput: {avg_throughput:.1f} plugins/s (should be > 400/s)"

        print("Manifest Parsing Performance:")
        print("  Parsing operations: 10")
...
class TestPluginInstallationPerformance:
    ...
    def test_single_plugin_installation_performance(
        self, plugin_benchmark_repo, benchmark_claude_env, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                for size, plugins in plugins_by_size.items():
                    ...
                    assert (
                        profiler.duration < expected_times[size]
                    ), f"{size} plugin install: {profiler.duration:.3f}s"
                ...
                print("Single Plugin Installation Performance:")
                for size, result in size_results.items():
                    print(
                        f"  {size.title()} plugin: {result['duration']:.3f}s ({'✓' if result['success'] else '✗'})"
                    )

    def test_batch_plugin_installation_performance(
        self, plugin_benchmark_repo, benchmark_claude_env, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                for batch_size in batch_sizes:
                    ...
                    assert (
                        profiler.duration < max_time
                    ), f"Batch {batch_size}: {profiler.duration:.3f}s (should be < {max_time}s)"
                    assert (
                        throughput > 5
                    ), f"Batch {batch_size} throughput: {throughput:.1f} plugins/s (should be > 5/s)"
                    assert result.get("success", False), f"Batch {batch_size} installation failed"
                    assert (
                        installed_count == batch_size
                    ), f"Batch {batch_size}: installed {installed_count} != {batch_size}"

                print("Batch Installation Performance:")
                for batch_size, result in batch_results.items():
                    print(
                        f"  {batch_size:3d} plugins: {result['duration']:6.3f}s, {result['throughput']:5.1f}/s, {result['memory_mb']:4.1f}MB"
                    )
                ...
                ), f"Throughput variation too high: {throughput_ratio:.1f}x"
    ...
    def test_plugin_update_performance(self, plugin_benchmark_repo, benchmark_claude_env, tmp_path):
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                plugins = repo_manager.discover_plugins(repo_dir)
                install_result = config_manager.install_plugins(
                    plugins[:20], repo_dir
                )  # Install 20 plugins
                ...
                assert (
                    profiler.duration < 2.0
                ), f"Update check: {profiler.duration:.3f}s (should be < 2s)"
                ...
                with PluginPerformanceProfiler("Update Execute") as profiler:
                    ...
                    update_result = config_manager.update_plugins(
                        update_info.updates_available, repo_dir
                    )
                    profiler.checkpoint("update_complete")

                assert (
                    profiler.duration < 5.0
                ), f"Update execute: {profiler.duration:.3f}s (should be < 5s)"
                ...
                update_throughput = (
                    updates_count / profiler.duration if profiler.duration > 0 else 0
                )
                assert (
                    update_throughput > 2
                ), f"Update throughput: {update_throughput:.1f} updates/s"

                print("Plugin Update Performance:")
...
class TestPluginConfigurationPerformance:
    ...
    def test_settings_file_performance(self, benchmark_claude_env):
        ...
        with PluginPerformanceProfiler("Settings File Operations") as profiler:
            for i in range(operations_count):
                ...
                config_manager._update_settings_atomic(
                    lambda s: s.setdefault("test_plugins", {}).update(
                        {
                            f"test_plugin_{i}": {
                                "enabled": i % 2 == 0,
                ...
                operation_times.append(
                    operation_time - (operation_times[-1] if operation_times else 0)
                )
        ...
        assert (
            profiler.duration < 10.0
        ), f"Total operations: {profiler.duration:.3f}s (should be < 10s)"
        assert (
            avg_operation_time < 0.05
        ), f"Average operation: {avg_operation_time:.3f}s (should be < 0.05s)"
        assert (
            max_operation_time < 0.2
        ), f"Max operation: {max_operation_time:.3f}s (should be < 0.2s)"

        operations_per_second = operations_count / profiler.duration
        assert (
            operations_per_second > 20
        ), f"Operations/sec: {operations_per_second:.1f} (should be > 20)"

        print("Settings File Performance:")
    ...
    def test_configuration_backup_performance(self, benchmark_claude_env):
        ...
        large_config = {
            "agents": {
                f"agent_{i}": {"path": f"/path/agent_{i}.md", "enabled": True} for i in range(50)
            },
            "commands": {
                f"cmd_{i}": {"path": f"/path/cmd_{i}.md", "enabled": True} for i in range(30)
            },
            "hooks": {
                f"hook_{i}": {"path": f"/path/hook_{i}.json", "enabled": True} for i in range(20)
            },
            "mcp": {
                "servers": {
                    f"server_{i}": {"command": "python", "args": ["-m", f"server_{i}"]}
                    for i in range(15)
                }
            },
        }
        ...
        assert (
            avg_restore_time < 0.5
        ), f"Average restore: {avg_restore_time:.3f}s (should be < 0.5s)"

        print("Configuration Backup Performance:")
        print("  Configuration size: 115 plugins")
...
class TestPluginMemoryEfficiency:
    ...
    def test_plugin_loading_memory_efficiency(
        self, plugin_benchmark_repo, benchmark_claude_env, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                for batch_start in range(0, len(all_plugins), batch_size):
                    ...
                    memory_checkpoints.append(
                        {
                            ...
                            "memory_delta": memory_delta,
                            "memory_per_plugin": memory_delta / plugins_loaded
                            if plugins_loaded > 0
                            else 0,
                        }
                    )
                    ...
                    memory_per_plugin_kb = (
                        (memory_delta / plugins_loaded) / 1024 if plugins_loaded > 0 else 0
                    )

                    assert (
                        memory_mb < 80
                    ), f"Memory usage: {memory_mb:.1f}MB for {plugins_loaded} plugins"
                    assert (
                        memory_per_plugin_kb < 300
                    ), f"Memory per plugin: {memory_per_plugin_kb:.1f}KB"
                ...
                print("Plugin Memory Efficiency:")
                ...
                for checkpoint in memory_checkpoints[::2]:  # Show every other checkpoint
                    ...
                    print(
                        f"    {plugins:3d} plugins: {memory_mb:5.1f}MB ({per_plugin_kb:4.1f}KB/plugin)"
                    )

    def test_concurrent_plugin_operations_memory(
        self, plugin_benchmark_repo, benchmark_claude_env, tmp_path
    ):
        ...
        with patch("os.getcwd", return_value=str(tmp_path)):
            with patch(
                ...
                assert (
                    max_memory_mb < 100
                ), f"Peak concurrent memory: {max_memory_mb:.1f}MB (should be < 100MB)"
                assert (
                    final_memory_mb < 30
                ), f"Final memory delta: {final_memory_mb:.1f}MB (should be < 30MB)"

                print("Concurrent Operations Memory:")
                print("  Workers: 4")
                print("  Operations per worker: 3 install/remove cycles")
