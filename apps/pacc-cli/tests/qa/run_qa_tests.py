...
class QATestRunner:
    ...
    def generate_report(self, all_results: Dict[str, Any]) -> str:
        ...
        passed_suites = sum(1 for r in all_results.values() 
        ...
        for suite_name, results in all_results.items():
            status = "✓ PASS" if (results.get('passed') or 
                                 results.get('summary', {}).get('suite_passed')) else "✗ FAIL"
            report.append(f"### {suite_name.replace('_', ' ').title()} {status}")
            
            # Add summary if available
            if 'summary' in results:
                summary = results['summary']
                report.append(f"- Tests: {summary.get('passed_tests', 0)}/{summary.get('total_tests', 0)}")
...
'''
    ...
    parser.add_argument(
        '--quick',
        action='store_true',
        help='Run quick test subset (unit, integration, build)'
    )
    
    parser.add_argument(
        '--cross-platform',
        action='store_true',
        help='Run only cross-platform tests'
    )
    
    parser.add_argument(
        '--package-managers',
        action='store_true',
        help='Run only package manager tests'
    )
    ...
    parser.add_argument(
        '--upgrade',
        action='store_true',
        help='Run only upgrade/uninstall tests'
    )
