...
class TestDocumentationCompleteness:
    ...
    def test_migration_guide_completeness(self, docs_dir):
        ...
        assert (
            "Development Installation → Global" in content or "Development to Global" in content
        ), "Missing development to global migration"
        assert (
            "Project-Specific → System-Wide" in content
            or "Project-Embedded to Standalone" in content
        ...
        assert (
            "Data Migration" in content or "Extension Migration" in content
    ...
    def test_getting_started_guide_completeness(self, docs_dir):
        ...
        assert (
            "5 minutes" in content.lower() or "five minutes" in content.lower()
        ...
        assert "Common Workflows" in content or "Workflows" in content, "Missing workflow examples"
                    ...
                    if line.strip().startswith("pacc "):  # Only check actual pacc commands
                        ...
                        if " " in line:
                            ...
                            if len(parts) > 1 and not parts[1].startswith("-"):
                                assert any(
                                    cmd in parts[1] for cmd in valid_commands
    ...
    def test_troubleshooting_guide_coverage(self, docs_dir):
        ...
        for issue in issues:
            assert (
                issue in content or issue.lower() in content.lower()
    ...
    def test_version_reporting_in_docs(self, docs_dir):
        ...
        for doc_file in [
            ...
            if guide_path.exists():
                ...
                assert (
                    "Version:" in content or "**Version**:" in content or "Last Updated:" in content
    ...
    def test_command_examples_coverage(self, docs_dir):
        ...
        for cmd in commands:
            ...
            assert "--" in content, "Missing flag examples"

    @pytest.mark.skipif(
        sys.platform.startswith("win"), reason="Shell script test for Unix-like systems"
    )
    def test_shell_scripts_in_docs_are_valid(self, docs_dir):
        ...
        for doc_file in docs_dir.glob("*.md"):
            ...
            for block in script_blocks:
                ...
                if not any(
                    keyword in block for keyword in ["#!/bin/bash", "if ", "for ", "while "]
                ):
                    continue
...
class TestDocumentationExamples:
    ...
    def test_complex_hook_example(self, tmp_path):
        ...
        hook_content = {
            ...
            "eventTypes": ["PreToolUse"],
            "matchers": [{"tool": "Write", "pathGlob": "**/*.{js,py,ts}"}],
            "commands": [
