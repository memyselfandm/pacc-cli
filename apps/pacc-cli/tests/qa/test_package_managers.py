...
class PackageManagerTester:
    ...
    def _detect_package_managers(self) -> Dict[str, bool]:
        ...
        try:
            result = subprocess.run(
                ["pip", "--version"], capture_output=True, text=True, check=False
            )
        ...
        try:
            result = subprocess.run(
                ["pipx", "--version"], capture_output=True, text=True, check=False
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                build_result = subprocess.run(
                    [str(pip_cmd), "install", "build"], capture_output=True, text=True, check=False
                )
                ...
                if build_result.returncode == 0:
                    ...
                    if result.returncode == 0:
                        ...
                        if wheel_files:
                            # Install from wheel
                            result = subprocess.run(
                                [str(pip_cmd), "install", str(wheel_files[0])],
                                capture_output=True,
                                text=True,
                                check=False,
                            )
    ...
    def test_uv_installation(self) -> Dict[str, any]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                result = subprocess.run(
                    ["uv", "pip", "install", "-e", str(self.pacc_root)],
    ...
    def test_pipx_installation(self) -> Dict[str, any]:
        ...
        try:
            ...
            subprocess.run(
                ["pipx", "uninstall", "pacc"], capture_output=True, text=True, check=False
            )

            # Test 1: Install from local path
            print("  Testing pipx installation...")
            result = subprocess.run(
                ["pipx", "install", str(self.pacc_root)],
                capture_output=True,
                text=True,
                check=False,
            )
            ...
            if result.returncode == 0:
                ...
                print("  Testing functionality...")
                result = subprocess.run(
                    ["pacc", "--version"], capture_output=True, text=True, check=False
                )
                results["tests"]["functionality"] = result.returncode == 0
                ...
                result = subprocess.run(
                    ["pipx", "upgrade", "pacc"], capture_output=True, text=True, check=False
                )
    ...
    def test_virtual_environments(self) -> Dict[str, any]:
        ...
        results = {"scenario": "virtual_environments", "tests": {}}
    ...
    def test_global_vs_local_installation(self) -> Dict[str, any]:
        ...
        results = {"scenario": "global_vs_local", "tests": {}}
