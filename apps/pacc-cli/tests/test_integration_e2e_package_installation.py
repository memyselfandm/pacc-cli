...
class TestEndToEndPackageInstallation:
    ...
    def test_wheel_installation_and_functionality(self, clean_build_env):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert version_result.returncode == 0, f"PACC version command failed: {version_result.stderr}"
            assert "1.0.0" in version_result.stdout, f"Version output unexpected: {version_result.stdout}"
    ...
    def test_cross_platform_compatibility(self, clean_build_env):
        ...
        for test_file in test_files:
            result = validator.validate_extension(test_file, {'.json', '.md', '.yaml', '.py', '.js'})
    ...
    def test_package_metadata_consistency(self, clean_build_env):
        ...
        assert init_version == pyproject_version, f"Version mismatch: pyproject.toml={pyproject_version}, __init__.py={init_version}"
    ...
    def test_comprehensive_functionality_after_install(self, clean_build_env):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "Package manager for Claude Code" in help_result.stdout or "PACC" in help_result.stdout
