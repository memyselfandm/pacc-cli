...
class TestPluginConvertCLI:
    ...
    def test_convert_command_with_repo_push(self, cli, temp_extension):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("pacc.cli.ExtensionToPluginConverter") as mock_converter_class, patch(
                "pacc.cli.PluginPusher"
                ...
                with patch("builtins.input", side_effect=["test-plugin", "Test Author"]):
                    ...
                    mock_pusher.push_plugin.assert_called_once_with(
                        output_dir / "test-plugin", "https://github.com/test/repo.git"
                    )
...
class TestPluginPushCLI:
    ...
    def temp_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            hooks_dir.mkdir()
            hooks_data = {
                "hooks": [{"name": "test", "eventTypes": ["PreToolUse"], "commands": ["echo test"]}]
            }
    ...
    def test_push_command_basic(self, cli, temp_plugin):
        ...
        with patch("pacc.cli.PluginPusher") as mock_pusher_class, patch.object(
            cli, "_confirm_action", return_value=True
        ):
            mock_pusher = Mock()
            mock_pusher_class.return_value = mock_pusher
            mock_pusher.push_plugin.return_value = True
            ...
            assert result == 0
            mock_pusher.push_plugin.assert_called_once_with(
                temp_plugin, "https://github.com/test/repo.git", private=False, auth_method="https"
            )
    ...
    def test_push_command_with_flags(self, cli, temp_plugin):
        ...
        with patch("pacc.cli.PluginPusher") as mock_pusher_class, patch.object(
            ...
            assert result == 0
            mock_pusher.push_plugin.assert_called_once_with(
                temp_plugin, "git@github.com:test/repo.git", private=True, auth_method="ssh"
            )
    ...
    def test_interactive_prompts(self, cli):
        ...
        try:
            with patch("pacc.cli.ExtensionToPluginConverter") as mock_converter_class:
                ...
                with patch(
                    "builtins.input", side_effect=["interactive-test", "Interactive Author"]
                ):
                    args = Mock()
