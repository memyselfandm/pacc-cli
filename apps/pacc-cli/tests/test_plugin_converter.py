...
from pacc.plugins.converter import (
    PluginConverter, ExtensionInfo, ConversionResult,
...
class TestPluginConverter:
    ...
    def test_convert_hooks_multiple_hooks(self, converter):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            hook_data2 = {
                "hooks": [
                    {"name": "hook2a", "eventTypes": ["PostToolUse"], "commands": ["echo 'test2a'"]},
                    {"name": "hook2b", "eventTypes": ["Notification"], "commands": ["echo 'test2b'"]}
                ]
            }
...
"""
    ...
    def test_convert_commands_preserves_structure(self, converter):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            source_path = Path(tmpdir) / "source" / ".claude" / "commands" / "subdir" / "test_cmd.md"
...
"""
    ...
    def test_generate_manifest_basic(self, converter):
        ...
        extensions_by_type = {
            ...
            "agents": [
                ExtensionInfo(Path("/agent1.md"), "agents", "agent1", is_valid=True)
            ]
        }
...
class TestConvertExtensionsToPlugin:
    ...
    def test_convert_extensions_to_plugin_success(self, mock_converter_class):
        ...
        mock_extensions = [
            ExtensionInfo(Path("/test.json"), "hooks", "test", is_valid=True)
        ]
    ...
    def test_file_permission_errors(self, converter):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('pathlib.Path.mkdir', side_effect=PermissionError("Permission denied")):
                result = converter._create_plugin_structure(plugin_path, ConversionResult(success=False))
