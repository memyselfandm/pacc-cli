...
from pacc.plugins.discovery import PluginInfo
...
class TestPluginRegistry:
    ...
    def test_get_recommendations_python_project(self):
        ...
        registry = PluginRegistry(self.registry_path)
        context = ProjectContext(project_type="python", languages={"python"}, has_tests=True)
        results = registry.get_recommendations(context, limit=10)
...
class TestLocalPluginIndex:
    ...
    def test_get_installed_plugins_with_repos(self, mock_config_manager, mock_scanner):
        ...
        mock_plugin_info = PluginInfo(
            name="test-plugin",
            path=Path("/fake/path"),
            manifest={"description": "A test plugin", "author": "test-author"},
            components={"commands": [Path("/fake/path/commands/test.md")]},
        )
...
class TestPluginSearchEngine:
    ...
    def test_sort_results_by_popularity(self, mock_local_index):
        ...
        results = [
            SearchResult(
                "low", "Low plugin", SearchPluginType.COMMAND, "url", "author", popularity_score=10
            ),
            SearchResult(
                "high",
                "High plugin",
...
class TestConvenienceFunctions:
    ...
    def test_get_plugin_recommendations(self):
        ...
        with patch("pacc.plugins.search.PluginSearchEngine") as mock_engine:
            ...
            mock_results = [
                SearchResult(
                    "recommended", "Recommended plugin", SearchPluginType.AGENT, "url", "author"
                )
            ]
...
class TestSearchPerformance:
    ...
    def test_search_performance_under_2_seconds(self):
        ...
        try:
            ...
            large_registry = {"version": "1.0", "plugins": plugins}
            ...
            assert (
                duration < 2.0
            ), f"Search took {duration:.2f} seconds, should be under 2.0 seconds"
