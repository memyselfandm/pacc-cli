...
""")
    ...
    @patch('pacc.plugins.repository.subprocess.run')
    def test_create_plugin_repository_success(self, mock_run, repo_manager, sample_plugin_structure):
        """Test successful plugin repository creation."""
        mock_run.return_value = Mock(returncode=0, stdout="", stderr="")
        
        # Read plugin metadata
        with open(sample_plugin_structure / "plugin.json", "r") as f:
            plugin_metadata = json.load(f)
        
        result = repo_manager.create_plugin_repository(
            sample_plugin_structure, 
            plugin_metadata,
            init_git=True
        )
    ...
    @patch('pacc.plugins.repository.subprocess.run')
    def test_create_plugin_repository_git_failure(self, mock_run, repo_manager, sample_plugin_structure):
    ...
    def test_commit_plugin_nothing_to_commit(self, mock_run, repo_manager, sample_plugin_structure):
        ...
        mock_run.side_effect = [
            Mock(returncode=0, stdout="", stderr=""),  # git add
            Mock(returncode=1, stdout="nothing to commit, working tree clean", stderr=""),  # git commit
        ]
    ...
    @patch('pacc.plugins.repository.subprocess.run')
    def test_push_plugin_update_existing_remote(self, mock_run, repo_manager, sample_plugin_structure):
    ...
    @patch('pacc.plugins.repository.subprocess.run')
    def test_push_plugin_authentication_failure(self, mock_run, repo_manager, sample_plugin_structure):
    ...
    @patch('pacc.plugins.repository.subprocess.run')
    def test_push_plugin_repository_not_found(self, mock_run, repo_manager, sample_plugin_structure):
...
class TestGitConversionWorkflows:
    ...
    def test_complete_conversion_workflow(self, mock_run, repo_manager, temp_plugins_dir):
        ...
        push_result = repo_manager.push_plugin(
            plugin_dir, 
            "https://github.com/user/workflow-plugin.git"
        )
