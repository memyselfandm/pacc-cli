...
class TestProjectConfigSchema:
    ...
    def test_valid_plugins_configuration(self):
        ...
        config = {
            ...
            "plugins": {
                "repositories": [
                    "owner/repo@v1.0.0",
                    {"repository": "another/repo", "version": "main"},
                ],
    ...
    def test_missing_required_fields(self):
        ...
        config = {"plugins": {"repositories": ["owner/repo"]}}
...
class TestConflictResolution:
    ...
    def test_version_conflict_detection(self):
        ...
        sources = [
            ConfigSource(
                ...
                config={"plugins": {"repositories": ["owner/repo@v1.0.0"]}},
                priority=10,
            ),
            ConfigSource(
                name="local",
                path=Path("/local/pacc.local.json"),
                config={"plugins": {"repositories": ["owner/repo@v1.1.0"]}},
                priority=20,
                is_local=True,
            ),
        ]
    ...
    def test_preferred_version_selection(self):
        ...
        result = self.sync_manager._choose_preferred_version("v1.0.0", "v1.1.0", "team", "local")
        assert result == "v1.1.0"

        # Specific version should beat dynamic
        result = self.sync_manager._choose_preferred_version("latest", "v1.2.3", "team", "global")
        assert result == "v1.2.3"

        # Higher version should win
        result = self.sync_manager._choose_preferred_version("v1.0.0", "v1.1.0", "team", "global")
        assert result == "v1.1.0"
...
class TestPluginSyncManager:
            ...
            json.dumps(
                {
                    "name": "test-project",
                    "version": "1.0.0",
                    "plugins": {"repositories": ["team/repo@v1.0.0"]},
                }
            )
        )

        (self.temp_dir / "pacc.local.json").write_text(
            json.dumps({"plugins": {"repositories": ["local/repo@v1.1.0"]}})
        )
            ...
            json.dumps(
                {
                    "name": "test-project",
                    "version": "1.0.0",
                    "plugins": {"repositories": ["owner/repo@v1.0.0"], "required": ["plugin1"]},
                }
            )
        )

        result = self.sync_manager.sync_plugins(project_dir=self.temp_dir, dry_run=True)
                    ...
                    json.dumps(
                        {
                            "name": "test-project",
                            "version": "1.0.0",
                            "plugins": {"repositories": ["owner/repo@v1.1.0"]},
                        }
                    )
                )
    ...
    def test_repository_spec_parsing(self):
        ...
        spec = self.sync_manager._parse_single_repository(
            {"repository": "owner/repo", "version": "main", "plugins": ["plugin1"]}
        )
        assert spec.repository == "owner/repo"
        assert spec.version == "main"
...
class TestPluginConfigManager:
    ...
    def setup_method(self):
        ...
        self.config_manager = PluginConfigManager(
            plugins_dir=self.temp_dir / "plugins", settings_path=self.temp_dir / "settings.json"
        )
...
class TestEndToEndIntegration:
    ...
    def test_complete_team_sync_workflow(self, mock_get_manager):
        ...
        local_config = {
            "plugins": {"repositories": ["local/dev-tools@latest"], "optional": ["dev-plugin"]}
        }
        ...
        result = self.sync_manager.sync_plugins_with_conflict_resolution(
            project_dir=self.temp_dir, conflict_resolution=conflict_resolution
        )
