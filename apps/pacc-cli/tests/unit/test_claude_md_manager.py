...
from pacc.errors.exceptions import FileSystemError, SecurityError, ValidationError
...
class TestAtomicOperations:
    ...
    def test_atomic_operation_rollback(self, manager, temp_dir):
        ...
        def mock_open_selective(*args, **kwargs):
            ...
            if (
                len(args) > 0
                and ".tmp" in str(args[0])
                and "w" in str(args[1] if len(args) > 1 else kwargs.get("mode", ""))
            ):
...
class TestThreadSafety:
    ...
    def test_concurrent_section_updates(self, manager, temp_dir):
        ...
        threads = []
        for i in range(10):
            thread = threading.Thread(target=update_section, args=(f"section{i}", f"Content {i}"))
        ...
        def update_same_section(thread_id):
            try:
                result = manager.update_section(
                    test_file, "shared_section", f"Content from thread {thread_id}"
                )
