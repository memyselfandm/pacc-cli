...
class TestCollectionMetadata:
    ...
    def test_metadata_to_dict(self):
        ...
        metadata = CollectionMetadata(
            name="test-collection", version="1.0.0", description="Test collection"
        )
...
class TestCollectionDependencyResolver:
    ...
    def test_resolve_missing_dependencies(self):
        ...
        metadata = CollectionMetadata(name="test", version="1.0.0", dependencies=["missing-dep"])
    ...
    def test_resolve_existing_dependencies(self):
        ...
        metadata = CollectionMetadata(name="test", version="1.0.0", dependencies=["existing-dep"])

        # Mock storage manager to return existing collection
        self.storage_manager.list_collections.return_value = {"existing-dep": ["fragment1"]}
...
class TestFragmentCollectionManager:
    ...
    def test_install_collection_actual(self, mock_storage_class):
        ...
        # Mock other managers
        with patch.object(self.collection_manager, "version_tracker"), patch.object(
            self.collection_manager, "_create_collection_backup", return_value={}
    ...
    def test_install_collection_selective_files(self):
        ...
        options = CollectionInstallOptions(selected_files=["fragment1"], dry_run=True)
        result = self.collection_manager.install_collection(collection_dir, options)
    ...
    def test_install_collection_with_dependencies(self):
        ...
        with patch.object(
            self.collection_manager.dependency_resolver, "resolve_dependencies"
        ) as mock_resolve:
    ...
    def test_get_collection_status_installed(self):
        ...
        with patch.object(self.collection_manager.storage_manager, "list_fragments") as mock_list:
            mock_list.return_value = [Mock(name="fragment1"), Mock(name="fragment2")]
    ...
    def test_remove_collection(self):
        ...
        # Mock storage manager
        with patch.object(
            self.collection_manager.storage_manager, "remove_collection"
        ) as mock_remove, patch.object(
            self.collection_manager, "_untrack_collection_installation"
        ) as mock_untrack:
    ...
    def test_update_collection(self):
        ...
        with patch.object(
            self.collection_manager.storage_manager, "list_collections"
        ) as mock_list, patch.object(self.collection_manager, "install_collection") as mock_install:
            ...
            options = CollectionInstallOptions(dry_run=True)
            result = self.collection_manager.update_collection(
                "test-collection", new_collection_dir, options
            )
