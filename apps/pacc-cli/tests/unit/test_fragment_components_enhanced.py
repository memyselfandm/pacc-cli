...
from pacc.fragments.storage_manager import FragmentStorageManager, FragmentLocation
from pacc.fragments.update_manager import FragmentUpdateManager, UpdateResult
...
class TestEnhancedFragmentValidator:
    ...
    def setup_method(self):
        ...
        self.deterministic_collection = SampleFragmentFactory.create_deterministic_collection(self.temp_dir)
    ...
    def test_validate_deterministic_fragments_consistency(self):
        ...
        for fragment_path in fragment_paths:
            ...
            for run in range(3):
                ...
                assert result.is_valid, f"Validation failed on run {run+1} for {fragment_path}: {result.errors}"
    ...
    def test_validate_edge_cases_consistently(self):
        ...
        for fragment_file, expected_type in edge_fragments:
            fragment_path = self.edge_case_collection / (
                "agents" if expected_type == "agent" else
                "commands" if expected_type == "command" else 
                "hooks"
            ) / fragment_file
            ...
            for run in range(3):
                result = self.validator.validate_single(fragment_path)
                assert result.is_valid, f"Edge case validation failed on run {run+1} for {fragment_file}"
                assert result.fragment_type == expected_type, \
...
"""
        ...
        for fragment_path, error_type in invalid_fragments:
            ...
            first_result = results[0]
            assert not first_result.is_valid, f"Invalid fragment {error_type} should fail validation"
...
class TestEnhancedFragmentStorageManager:
    ...
    def test_storage_locations_deterministic(self):
        ...
        assert self.project_root in project_location.base_path.parents or project_location.base_path == self.project_root
        ...
        assert self.user_home in user_location.base_path.parents or user_location.base_path == self.user_home
...
class TestEnhancedFragmentInstallationManager:
    ...
    def setup_method(self):
        ...
        # Create sample collections
        self.deterministic_collection = SampleFragmentFactory.create_deterministic_collection(self.temp_dir)
        self.dependency_collection = SampleFragmentFactory.create_dependency_collection(self.temp_dir)
...
"""
    ...
    def test_installation_result_consistency(self):
        ...
        for i, result in enumerate(results[1:], 1):
            ...
            assert set(result.installed_fragments.keys()) == set(first_result.installed_fragments.keys())
    ...
    def test_dependency_resolution_consistency(self):
        ...
        for run in range(3):
            ...
            result = self.installation_manager.install_from_source(
                str(self.dependency_collection),
                target_type="project",
                install_all=True
            )
            results.append(result)
    ...
    def test_source_resolution_consistency(self):
        ...
        sources_to_test = [
            ...
            (str(self.deterministic_collection / "agents" / "test-simple-agent.md"), "local", False),
        ]
        ...
        for source_input, expected_type, is_collection in sources_to_test:
            ...
            for run in range(3):
                ...
                assert resolved.is_collection == is_collection, f"Wrong collection flag on run {run+1}"
...
"""
        ...
        pacc_config = {
            "name": "fragment-integration-test",
            "version": "1.0.0", 
            "fragments": {}
        }
        (self.project_root / "pacc.json").write_text(json.dumps(pacc_config, indent=2))
    ...
    def test_full_workflow_reliability(self):
        ...
        for run in range(3):
            ...
            fragment_paths.extend([p for p in collection_path.rglob("*.json") if "collection" not in p.name])
            ...
            assert install_result.installed_count == len(validated_fragments), \
                f"Install count mismatch on run {run+1}"
            
            # 3. Verify storage
            stored_fragments = self.storage_manager.list_installed_fragments()
            assert len(stored_fragments) == install_result.installed_count, \
    ...
    def test_cross_component_data_integrity(self):
        ...
        for fragment_name in installed_names:
            ...
            assert retrieved["name"] == fragment_name, f"Retrieved fragment name mismatch"
