...
class TestURLDownloader:
    ...
    def setup_method(self):
        """Setup test method."""
        self.downloader = URLDownloader(max_file_size_mb=10, timeout_seconds=30)
    ...
    async def test_download_with_progress_callback(self):
        ...
        with patch("pacc.core.url_downloader.aiohttp.ClientSession") as mock_session_class:
            ...
            chunk_size = 250
            chunks = [
                mock_response_data[i : i + chunk_size]
                for i in range(0, len(mock_response_data), chunk_size)
            ]
