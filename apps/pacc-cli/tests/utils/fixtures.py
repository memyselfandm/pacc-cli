...
"""
                ...
                content = {"name": plugin_name, "command": "python", "args": ["-m", f"test_{i}"]}
    ...
    def create_sized_repo(
        tmp_path: Path, small_count: int = 10, medium_count: int = 10, large_count: int = 5
    ) -> Path:
        ...
        for size, count, multiplier in plugin_configs:
            for i in range(count):
                ...
                if category in ["agents", "commands"]:
                    feature_text = "- Feature implementation with detailed description\n" * (
                        3 * multiplier
                    )
                    usage_text = (
...
"""
        ...
        manifest = {
            ...
            "description": "Test repository with versioned plugins",
            "versions": versions,
            "plugins": plugins_list,
        }
...
class TeamWorkspaceFactory:
    ...
    def create_team_workspace(
        tmp_path: Path, member_names: List[str], shared_plugins: Optional[List[str]] = None
    ) -> Dict[str, Path]:
        ...
        for member_name in member_names:
            ...
            settings = {
                ...
                "commands": {},
                "mcp": {"servers": {}},
                "team": {"member_name": member_name, "workspace": str(member_dir)},
            }
...
"""
                ...
                content = {
                    ...
                    "team_config": {"shared": True, "collaboration": True},
                }
        ...
        (shared_repo / "pacc-manifest.yaml").write_text(
...
class ClaudeEnvironmentFactory:
    ...
    def create_performance_environment(tmp_path: Path) -> Path:
        ...
        settings.update(
            {
                "performance": {"mode": "testing", "enable_metrics": True, "cache_plugins": True},
                "benchmark": {"start_time": time.time(), "test_mode": True},
            }
        )
        settings_file.write_text(json.dumps(settings, indent=2))
...
def create_test_plugin(
    plugin_type: str, name: str, size: str = "medium", version: str = "1.0.0", **kwargs
) -> Dict[str, Any]:
...
def create_test_manifest(
    name: str, plugins: List[Dict[str, Any]], version: str = "1.0.0", **kwargs
) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugins": plugins,
        "test_config": {"created_at": time.time(), "test_mode": True},
        **kwargs,
    }
