...
class MockClaudeEnvironment:
    ...
    def add_plugin(self, plugin_type: str, plugin_name: str, plugin_path: str):
        ...
        if plugin_type == "mcp":
            ...
            self.settings[plugin_type][plugin_name] = {
                "path": plugin_path,
                "enabled": True
            }
        self.save_settings()
...
def patch_file_system(mock_fs: MockFileSystem):
    ...
    with patch('pathlib.Path.exists') as mock_exists, \
        ...
        mock_write.side_effect = lambda self, content, **kwargs: mock_fs.write_text(str(self), content)
...
def patch_environment(mock_env: MockEnvironment):
    ...
    with patch('os.environ', mock_env.env_vars), \
         patch('platform.system') as mock_platform, \
         patch('os.name', mock_env.platform):
